...
class paramData:
    ...
    mining_payoff_mean: float = None
    mining_payoff_variance: float = None
    t_stat: float = None
    crit_value: float = None
    p_value: float = None

    def __init__(self,passive:tuple,bid:tuple,mining_mean,mining_var,ttest:tuple):
        self.passive_tech = passive
        self.bid_tech = bid
        self.mining_payoff_mean = mining_mean
        self.mining_payoff_variance = mining_var
        self.t_stat = ttest[0]
        self.crit_value = ttest[1]
        self.p_value = ttest[2]

    def to_series(self):
        data=[getTimeNow(),self.passive_tech,self.bid_tech,self.mining_payoff_mean,self.mining_payoff_variance,self.t_stat,self.crit_value,self.p_value]
        df=pd.Series(data,index=['date','passive_tech','bid_tech','mining_payoff_mean','mining_payoff_variance','t_stat','critical_value','p_value'])
        return df
...
def analyze(sample_df : pd.DataFrame):
        ...
        if sample_df.size > 0 :
            ...
            ttest_result=TTest(analysis.old_sample['tech_gain_passive'].dropna(),analysis.new_sample['tech_gain_passive'].dropna())
            ###### END base_reward ######
            ...
            shape,floc,fscale = stats.lognorm.fit(mining_payoff)
            ...
            return_data = paramData(getUniformParams(won_tech_passive),getUniformParams(won_tech_bid),p_mean,p_var,ttest_result)
            return_data.to_file()
